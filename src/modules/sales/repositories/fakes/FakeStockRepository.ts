...
class FakeStockRepository implements IStockRepository {
  ...
  public async listStock(provider_id: string): Promise<Stock[]> {
    const listStock = this.stock.filter(stock => stock.provider_id === provider_id)
  ...
  public async matchProduct({ product, provider_id }: IProductsInfoDTO): Promise<Stock | undefined> {
    const match = this.stock.find(stock => stock.provider_id === provider_id && stock.product === product)

    return match;
  }

  public async save(stock: Stock): Promise<Stock> {
    const findIndex = this.stock.findIndex(findStock => findStock.product === stock.product);
